          ...
          {MODES.map(mode => (
            <motion.button key={mode.id} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setActiveMode(mode.id)} disabled={mode.comingSoon}
              className={cn("group flex-shrink-0 flex flex-col items-center gap-1.5 px-4 py-3 rounded-2xl border transition-all duration-200 min-w-[95px] snap-start shadow-sm",
                activeMode === mode.id
                  ? "bg-blue-600 dark:bg-cyan-600 border-blue-500 dark:border-cyan-400 text-white shadow-lg shadow-blue-600/30 dark:shadow-cyan-500/30 font-bold"
                  : "bg-blue-500/10 dark:bg-cyan-500/15 border-blue-500/30 dark:border-cyan-500/30 text-blue-700 dark:text-cyan-300 hover:bg-blue-600 dark:hover:bg-cyan-600 hover:text-white hover:border-blue-600 dark:hover:border-cyan-400",
                mode.comingSoon && "opacity-40 cursor-not-allowed")}>
              <mode.icon className={cn("w-5 h-5", activeMode === mode.id ? "text-white" : "text-blue-600 dark:text-cyan-400 group-hover:text-white")} />
              <span className={cn("text-[10px] font-bold uppercase tracking-wide", activeMode === mode.id ? "text-white" : "text-blue-700 dark:text-cyan-200 group-hover:text-white")}>{mode.label}</span>
            </motion.button>
          ))}
        </motion.div>
      </div>

      <motion.div layout={!isGenerating} className={cn("min-h-[320px] rounded-[2rem] md:rounded-3xl border bg-white dark:bg-[#1e1a4d] border-blue-100 dark:border-cyan-500/30 p-4 md:p-6 flex flex-col w-full max-w-full relative shadow-md dark:shadow-xl dark:shadow-cyan-500/5", currentMode.bg, currentMode.border)}>
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-5 text-center sm:text-left">
          ...
          <div className="flex items-center gap-2">
            ...
            {hasOutput && !isGenerating && (
              <>
                {activeMode === "report" && (
                  <button onClick={copyToClipboard} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 dark:bg-cyan-600 dark:hover:bg-cyan-500 text-white border border-blue-500 dark:border-cyan-400 text-xs font-bold transition-colors group shadow-sm">
                    {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                    {copied ? "Copied!" : "Copy for Substack"}
                  </button>
                )}
                <button onClick={onManualSave} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 dark:bg-cyan-600 dark:hover:bg-cyan-500 text-white border border-blue-500 dark:border-cyan-400 text-xs font-bold transition-colors group shadow-sm"><Save className="w-3.5 h-3.5" /> Save</button>
                <button onClick={() => generate()} className="p-2 rounded-xl bg-blue-600 hover:bg-blue-700 dark:bg-cyan-600 dark:hover:bg-cyan-500 text-white border border-blue-500 dark:border-cyan-400 transition-colors shadow-sm"><RefreshCw className="w-4 h-4" /></button>
              </>
            )}
            {streamText && !isGenerating && <button onClick={() => downloadReportOrText(streamText, `workspaceiq-${activeMode}.md`)} className="p-2 rounded-xl bg-blue-600 hover:bg-blue-700 dark:bg-cyan-600 dark:hover:bg-cyan-500 text-white border border-blue-500 dark:border-cyan-400 transition-colors shadow-sm"><Download className="w-4 h-4" /></button>}
          </div>
        </div>
              ...
              <motion.div key="empty" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="h-full flex flex-col items-center justify-center gap-4 text-center py-6">
                ...
                {activeMode === "report" && (
                  <div className="flex flex-col sm:flex-row gap-3 w-full max-w-md mx-auto my-2">
                    <button
                      ...
                      className={cn(
                        ...
                        selectedReportType === "standard"
                          ? "bg-blue-600 dark:bg-cyan-600 text-white border-blue-500 dark:border-cyan-400 shadow-md font-bold"
                    ...
                    >
                      <div className={cn("text-xs font-black uppercase tracking-wider mb-1", selectedReportType === "standard" ? "text-white" : "text-blue-600 dark:text-cyan-400")}>Standard Report</div>
                    ...
                    >
                      <div className={cn("text-xs font-black uppercase tracking-wider mb-1", selectedReportType === "deep-dive" ? "text-white" : "text-cyan-400")}>Deep Dive Report</div>
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => generate()} disabled={!sources.length || currentMode.comingSoon} className={cn("flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm transition-all shadow-lg bg-blue-600 hover:bg-blue-700 dark:bg-cyan-600 dark:hover:bg-cyan-500 text-white border border-blue-500 dark:border-cyan-400 shadow-blue-600/20 dark:shadow-cyan-500/20", (!sources.length || currentMode.comingSoon) && "opacity-40 cursor-not-allowed")}><Sparkles className="w-4 h-4" /> Generate</motion.button>
              </motion.div>
            )}
